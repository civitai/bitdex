...
All test data goes under `.test-data/` in the project root. This directory is gitignored.

**Never create test data directories in the project root.** Use `.test-data/<purpose>/` instead:
- `.test-data/e2e/` — used by the automated E2E runner
- `.test-data/manual/` — for manual test server runs
- `.test-data/bench/` — for benchmark data

```bash
# Good
cargo run --release --features server --bin bitdex-server -- --port 3100 --data-dir .test-data/manual

# Bad — clutters root, might not be gitignored
cargo run --release --features server --bin bitdex-server -- --port 3100 --data-dir ./test-data
cargo run --release --features server --bin bitdex-server -- --port 3100 --data-dir ./my-test
```
...
### Microbenchmarks
For throwaway performance experiments, use the scratch crate. Run `/microbench` for the guide.
...
See the "E2E Coverage Gap Analysis" section in `docs/guide/testing.md` for prioritized missing scenarios and suggested new test suites.
