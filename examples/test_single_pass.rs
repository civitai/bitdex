...
//!
//! Usage: cargo run --release --features pg-sync --example test_single_pass -- [--threads N]
//!
...
fn main() {
    ...
    let data_dir = PathBuf::from("data/test-single-pass");
    ...
    let result = {
        if phase2_only {
            eprintln!("Note: --phase2-only is ignored in single-pass mode (no gather phase)");
        }
        single_pass::run_single_pass_v2(&engine, &index_def, &stage_dir, progress)
    };
