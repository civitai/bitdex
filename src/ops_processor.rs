...
pub fn process_csv_dump_direct(
    ...
    // Chunked reader: sends ~300MB blocks of complete lines through a channel.
    // Same pattern as the single-pass loader. Never loads full file into memory.
    fn spawn_block_reader(
        path: std::path::PathBuf,
        target_bytes: usize,
    ) -> (std::sync::mpsc::Receiver<Vec<u8>>, thread::JoinHandle<()>) {
        ...
        let handle = thread::spawn(move || {
            ...
            let mut accum = Vec::<u8>::with_capacity(target_bytes + 4 * 1024 * 1024);
            ...
            loop {
                ...
                if accum.len() >= target_bytes {
                    // Split at last newline to keep lines intact
                    if let Some(last_nl) = accum.iter().rposition(|&b| b == b'\n') {
                        let remainder = accum[last_nl + 1..].to_vec();
                        accum.truncate(last_nl + 1);
                        let batch = std::mem::replace(
                            &mut accum,
                            Vec::with_capacity(target_bytes + 4 * 1024 * 1024),
                        );
                        accum = remainder;
    ...
    let target_block_bytes = 300 * 1024 * 1024; // ~300MB per block

    // Phase 1: Images (creates alive slots)
    let images_csv = csv_dir.join("images.csv");
    if images_csv.exists() {
        let img_start = Instant::now();
        let (rx, reader_handle) = spawn_block_reader(images_csv, target_block_bytes);
        ...
        while let Ok(block) = rx.recv() {
            if phase_total >= record_limit { break; }

            let block_str = match std::str::from_utf8(&block) {
                Ok(s) => s,
                Err(_) => continue,
            };
            let mut lines: Vec<&[u8]> = block_str
                .split('\n')
                .filter(|l| !l.is_empty())
                .map(|l| l.as_bytes())
                .collect();
        ...
        eprintln!("  images: {} rows, {:.1}s ({:.0}/s)",
            phase_total,
            img_start.elapsed().as_secs_f64(),
            phase_total as f64 / img_start.elapsed().as_secs_f64().max(0.001));
    }

    // Phase 2: Tags (chunked reader + rayon)
    let tags_csv = csv_dir.join("tags.csv");
    if tags_csv.exists() {
        let tag_start = Instant::now();
        let (rx, reader_handle) = spawn_block_reader(tags_csv, target_block_bytes);
    ...
    // Phase 3: Tools (chunked reader + rayon)
    let tools_csv = csv_dir.join("tools.csv");
    if tools_csv.exists() {
        let tool_start = Instant::now();
        let (rx, reader_handle) = spawn_block_reader(tools_csv, target_block_bytes);
