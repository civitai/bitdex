...
Initial load and steady-state sync use the same WAL infrastructure. Two channels feed the same WAL reader:

- **Dump channel**: pg-sync writes table dumps directly to WAL files on the shared filesystem (no HTTP overhead)
- **Ops channel**: steady-state ops arrive via `POST /ops` → WAL append
...
K8s starts pod (BitDex server + bitdex-sync sidecar)
  ...
  → For each sync_source not yet dumped:
      1. PUT /api/indexes/{name}/dumps — register dump, returns task ID
      2. COPY table from PG (flat, no JOINs on large tables)
      3. Transform rows → ops using config schema
      4. Write ops directly to WAL file on shared filesystem
      5. POST /api/indexes/{name}/dumps/{name}/loaded — signal file complete
      6. BitDex WAL reader processes dump, saves bitmaps to disk, unloads from memory
  → Seed cursor at current max(BitdexOps.id)
...
Dump WAL processing bypasses the normal write path (coalescer → flush → snapshot → cache). Instead:

- Builds bitmaps directly in memory (like the single-pass loader)
- Appends docstore entries (no read-back needed — everything is new)
- Skips snapshot publishing, cache invalidation, bound maintenance
- On dump completion: save bitmaps to disk via ShardStore, unload from memory
- Lazy load on first query (existing `ensure_fields_loaded()` path)

This keeps peak memory low — only one table's bitmaps in memory at a time.

K8s readiness probe returns 503 during dumps (health probe stays 200). Traffic routes only after all dumps complete.
...
| Effective throughput | ~2,500 changes/s | 50,000–100,000 ops/s |
| 80M gap catch-up | ~9 hours | N/A (fresh load) |
| Fan-out cost (15M images) | 15M enrichment queries | 2 bitmap ops |
| Steady-state headroom | Negative (falls behind) | 20-40x over peak traffic |
