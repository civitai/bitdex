...
- [ ] **PG-sync sidecar UID 65532 PVC permission cleared by Tom.** PodSecurity adjusts pg-sync to `runAsUser: 65532`; existing `data-bitdex-0` PVC may have files owned by a different UID from prior deploys. Tom is validating; do not apply until cleared. Mitigations available if needed: `initContainer` chown, `fsGroup: 65532`, or leave pg-sync as the prior UID. Manifest patch §header notes this gate.
