...
| 11 | `sortAtUnix` | `sortAtUnix` | `single_value` | Integer | Unix timestamp **seconds** (converted from ms via `ms_to_seconds: true` in data_schema). Stored as seconds to fit in u32 cleanly and align with the `sortAt` sort field semantics. **Range filter thresholds must be passed as seconds-since-epoch.** Registered as `filter_only` in data_schema (sibling to the `sortAt` sort target derived from the same source). Prior to 2026-04-30 this field was declared in `time_buckets.filter_field` but missing from `filter_fields` — Gte/Lte clauses silently passed all rows because the executor's `range_scan` got `FieldNotFound` and the cache's `slot_matches_clause` returned `true` conservatively (`unified_cache.rs:2129`). |
